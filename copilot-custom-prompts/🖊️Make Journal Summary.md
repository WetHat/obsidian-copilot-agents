...
- Output must:
	...
	- structure group content as numbered list where each list item starts with a note reference (wikilink) followed by dense high-density summary with highlights of operational/practical details (indented bullet list with max. 3 items) for each note in this category 
...
- cites refrenced notes as Wikilinks 
- references notes only once per group
